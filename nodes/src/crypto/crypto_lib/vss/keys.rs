use crate::{
    crypto::crypto_lib::{
        ...
        vss::public_parameters::PublicParameters,
        {random_scalar, random_scalars, G1_PROJ_NUM_BYTES, SCALAR_NUM_BYTES},
    },
...
//
impl InputSecret {
    pub fn new_random<R>(t: usize, pedersen: bool, rng: &mut R) -> Self
    where
        R: rand_core::RngCore + rand::Rng + rand_core::CryptoRng + rand::CryptoRng,
    {
        let a = random_scalar(rng);
        let mut f = random_scalars(t, rng);

        // Decides to add randomness depending upon the value of the pedersen flag
        let mut r = random_scalars(t, rng);
        if !pedersen {
            for scalar in r.iter_mut().take(t) {
                *scalar = Scalar::zero();
            }
        }
        ...
        debug_assert!(f.len() == t);
        debug_assert_eq!(poly_eval(&f, &Scalar::zero()), a);

        InputSecret {
            inner: InputSecretInner { a, f, r },
        }
    }
...
impl DealtSecretKey {
    ...
    pub fn reconstruct(sc: &Configuration, shares: &[(u16, DealtSecretKeyShare)]) -> Self {
        assert_ge!(shares.len(), sc.get_threshold());
        assert_le!(shares.len(), sc.n() as usize);
