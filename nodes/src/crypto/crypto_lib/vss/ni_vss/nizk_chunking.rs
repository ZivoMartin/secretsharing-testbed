...
use super::{
    ...
    encryption::CiphertextChunks,
    fiat_shamir::{FiatShamirProtocol, NIVSS_DOM_SEP},
    utils::{batch_mul, get_xpowers, scalar_mult_exp, scalar_usize_mult_exp},
};
use crate::crypto::crypto_lib::{hash_to_scalar, random_scalars, util};
use std::ops::{Mul, Neg};
...
use rand_chacha::{
    rand_core::{RngCore as CRngCore, SeedableRng},
    ChaCha20Rng,
};
...
impl FirstMoveChunking {
    pub fn new(y0: G1Projective, bb: Vec<G1Projective>, cc: Vec<G1Projective>) -> Self {
...
pub fn prove_chunking(
    ...
    let (first_move, first_challenge, z_s) = loop {
        let sigma = [(); NUM_ZK_REPETITIONS].map(|_| random_within_range(range as u64) + p_sub_s);
        ...
        let cc = {
            ...
            for i in 0..NUM_ZK_REPETITIONS {
                let b = beta[i];
                let s = sigma[i];
                cc.push(G1Projective::multi_exp(
                    y0_g1_tbl.as_slice(),
                    [b, s].as_slice(),
                ));
            }
        ...
        let first_challenge =
            ChunksOracle::new(public_keys, ciphertexts, &first_move).get_all_chunks(n, m);
        ...
        let zs_in_range =
            z_s.iter().map(|z| zz_big.gt(z) as isize).sum::<isize>() as usize == NUM_ZK_REPETITIONS;
    ...
    let yy = {
        let y0_and_pk = [y0].iter().chain(public_keys).cloned().collect::<Vec<_>>();
...
pub fn verify_chunking(
    ...
    */
    ...
    let cij_to_eijks: Vec<G1Projective> = (0..NUM_ZK_REPETITIONS)
        .map(|k| {
            let c_ij_s: Vec<_> = ciphertexts.cc.iter().flatten().cloned().collect();
            let e_ijk_s: Vec<_> = e
                .iter()
                .flatten()
    ...
    let rhs = G1Projective::multi_exp(public_keys, &nizk.z_r)
        + G1Projective::multi_exp([nizk.y0, g1].as_slice(), [nizk.z_beta, acc].as_slice());
...
impl ChunksOracle {
    pub fn new(
        public_keys: &[G1Projective],
        cxts: &CiphertextChunks,
        first_move: &FirstMoveChunking,
    ) -> Self {
        let mut t = merlin::Transcript::new(NIVSS_DOM_SEP);

        t.append_encryption_keys(public_keys);
